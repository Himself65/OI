...
ll a[maxn];
inline void push_up_sum(ll p) { ans[p] = ans[LEFT_SON(p)] + ans[RIGHT_SON(p)]; }
...
inline void update(ll l, ll r, ll p, ll nl, ll nr, ll k) {
  ...
  push_down(l, r, p);
  ll mid = (l + r) >> 1;
  if (nl <= mid) update(l, mid, LEFT_SON(p), nl, nr, k);
  if (nr > mid) update(mid + 1, r, RIGHT_SON(p), nl, nr, k);
  push_up_sum(p);
}
inline ll query(ll l, ll r, ll p, ll qx, ll qy) {
  ...
  ll mid = (l + r) >> 1;
  push_down(l, r, p);
  if (qx <= mid) res += query(l, mid, LEFT_SON(p), qx, qy);
  if (qy > mid) res += query(mid + 1, r, RIGHT_SON(p), qx, qy);
  return res;
}
int main() {
  INIT_CIN;
